...
[English README](README.md) · [首次使用指南](docs/first-run.md#中文操作流程) · [可视化导入指南](docs/visual-import.md#中文操作说明) ·
[可视化连接面板](docs/configuration-dashboard.md) ·
[本地 AI 接入](docs/local-ai.md) ·
[Agent 工作流](docs/agent-workflow.md) · [浏览器选品队列](docs/browser-queue-workflow.md)
...
打开 **CatalogFlow.exe**，先看“新手开始”：选择 Codex 或 Claude，点击“检查安装与登录”；
缺少程序时按官方说明安装，需要登录时点击“打开官方登录”。支持的 Windows CLI 可打开登录窗口，
其余情况会显示供你自行运行的命令；软件不会代装 CLI 或代替你完成账号登录。
可自行确认并点击小型示例 AI 测试，验证一次实际调用；此操作可能消耗
账号用量。**保存连接名称、找到程序或检测到已登录，都不等于模型已经成功运行。**

可以直接粘贴 CJ 商品链接，也可以点击“开始采集”，在兼容的浏览器扩展中安装配套脚本并复制配对码。
到支持的商品详情页点击 **Add to CatalogFlow**，确认发送内容后，在提示框粘贴整行配对码。
配对只保存在当前网页内存中；新标签页、加载另一个商品页面或刷新后，都要重新粘贴。
CJ 和 Alibaba 国际站的已选链接会进入“已采集商品”；先点击“结束采集并确认”，再选择要处理的
商品。CJ 通过使用者自己的官方 API 获取资料；Alibaba 暂无自动取数适配器，会打开手动商品表单，
补充成本、运费、规格及授权图片后再生成预览，**不要求小白编写 JSON 文件**。高级 JSON 和旧队列文件
仍可导入。

选择 AI 后点击“生成预览”，程序会自动把商品任务交给本机 CLI，并把结果显示回来，
**不用再去 Codex/Claude 聊天窗口发一句“可以开始了”**。不使用 AI 时可选本机模板。
只做预览无需店铺；真正使用时，使用者自行配置自己的店铺并在审核后明确创建隐藏草稿。
详见[首次使用指南](docs/first-run.md#中文操作流程)。
...
- **浏览器内审核单个商品。** 输入 CJ 链接/PID、手动填写商品表单或选择授权 JSON 文件，查看每个变体的
  成本、运费、USD 售价并编辑文案。
...
预览可以反复生成；同一店铺中已经完成的导入会按来源及商品编号跳过。创建草稿过程中断或结果
不确定时，登记会保留并阻止直接重建，需先核对已有草稿。旧私人目录中的历史数据不会自动导入。
图片和变体导出保留各自属性与价格；上传图片还需在 WooCommerce 连接中填写具有媒体上传权限的
WordPress 用户名和**应用程序密码**，密码保存在系统凭据库。无图片时可留空。商品保持隐藏草稿，
已上传媒体文件本身可能通过店铺服务器的直接链接访问。详见[工作报告说明](docs/work-reports.md)。
...
预览版，Alibaba/1688 和 Zendrop 自动 API 连接器仍在开发中；获授权的 Alibaba/1688 商品事实
已经可以通过手动表单输入。完整说明见
...
- **方案 B：自定义成本公式。** 可直接在方案卡片里填写 `*5`、`/5`、`+2`、`-2`、`*3+2`
  或 `*(5+2)/3`，默认 `*3`，只填 `5` 等同 `*5`。公式作用于商品成本，先乘除后加减，
  再加一次运费、预估税费/关税和固定支付费。页面展示计算过程，最低售价和 `.95` 尾数仍生效。

方案 B 的售价不使用百分比费用或预留，但预估利润会扣除这些项目；它不会自动达到目标利润率，
公式结果过低时可能亏损。这里的预估利润仍扣除原始商品成本，仅覆盖已填写的成本与预留，不是会计净利润。

公式支持数字、`+ - * /` 和括号，最多 120 个字符；除以零、负数结果或过大结果会明确报错。
切换语言保留公式，只有点击保存才会用于后续每个商品变体的预览。旧倍数设置可继续读取；
新保存文件使用版本 2，旧版软件无法读取该格式。定价公式不会发送给 AI。
...
目标金额、汇率、来源和参考日期。三个位置依次为**目标货币、可修改汇率、换算总额**。
中英文标签换行时，三栏仍保持等宽、框体等高并对齐；窄屏自动改为单列。
Frankfurter 每日参考汇率自动填入第二栏，可直接修改或自己填写，修改后自动标为手动汇率；
...
1. 选择 **CJ 商品链接/PID**及已保存的 CJ 连接，或者选择“手动填写商品 / JSON”中的“直接填写表单”，
   输入已核实事实、变体、USD 成本和每件运费。“高级：导入规范化 JSON”仍支持单个 `alibaba-manual`
   商品对象（UTF-8，可带 BOM，最多 48 KiB）。已确认的浏览器选品可填入 CJ 输入框或手动表单的来源信息。
...
图形界面可以选择“手动填写商品 / JSON”→“直接填写表单”，无需编写 JSON。填写商品标题、
稳定的商品编号、已核实事实、变体、USD 成本及每件运费；授权图片链接可选。事实按每行
“名称: 内容”填写，多属性变体可以使用高级 JSON。

命令行与高级文件入口使用下方格式的规范化 JSON。输入必须来自你自己拥有或获授权的资料、
明确导出文件或本人获准使用的官方 API。浏览器选品器只排队 URL 和标题，不会自动补齐商品事实。
商品文件不能包含 Cookie、API Key、账号密码或客户信息。
...
## 在 CJ / Alibaba 浏览器页面选品

桌面界面已经把浏览器选品接到单商品导入向导：

```text
开始采集 → 在支持的商品页面配对 → Add to CatalogFlow
→ 核对本机队列 → 结束采集并确认 → 选择一个商品
→ CJ 官方 API 或手动补充事实 → 生成预览 → 审核 → 按需创建隐藏草稿
```

配套脚本支持 `www.cjdropshipping.com`、`cjdropshipping.com` 的 CJ 商品详情页，以及
`www.alibaba.com` 的 Alibaba 国际站商品详情页；不支持 1688 页面或搜索结果页采集。
点击“开始采集”，再点击“安装 / 更新采集脚本”和“复制配对码”。浏览器没有用户脚本管理器时，
先自行安装兼容扩展。到商品页点击 **Add to CatalogFlow**，确认显示的链接和标题，再按提示配对。

整行配对码一次带入本机接收地址和临时采集令牌，无需分别填写。脚本只在当前网页内存中记住它们，
不会跨标签页、重新加载的页面或刷新保留；同一采集器仍运行时，可在另一页粘贴相同配对码。
重新开始一轮采集后必须使用新配对码；配对信息不会写入扩展同步存储。

回到“已采集商品”核对列表，点击“结束采集并确认”。CJ 商品可点击“填入搬运向导”，生成预览时
使用你选定的 CJ 官方 API 连接获取资料。Alibaba 商品可点击“手动补充商品资料”，填写缺少的事实、
成本、运费和授权图片，也可使用规范化 JSON；Alibaba 自动取数适配器仍未实现。采集与确认队列都不会
调用 AI 或写入店铺。
...
命令会显示本机脚本安装地址、整行配对码和队列文件位置。使用同一脚本完成选品后，在终端按 Enter
冻结队列并停止采集；Enter 不会开始 AI 生成。再打开面板选择已确认商品；如果面板早于这次 CLI
采集启动，可先退出服务再重开，或通过“已有采集队列文件？”导入这份快照（最多 48 KiB）。
采集队列文件和规范化商品 JSON 是不同格式，须使用各自的入口。

采集请求只包含协议版本、来源、商品详情 URL 和页面标题，不发送整页 HTML、Cookie、图片、价格、
变体或登录信息。关闭程序或取消命令行采集只会停止接收，不会自动确认未完成的队列；
重开程序后可明确确认已保存的未完成队列。
