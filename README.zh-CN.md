...
同一个本机面板里有“可视化定价方案”窗口。它会把成本拆解、公式和实时试算结果显示出来，
在写入商店前先让你检查。提供两种确定性方案：
...
面板可以修改倍数和示例成本，实时比较两种结果，再把当前方案保存到本机。设置文件是与连接
档案并列的非秘密 `pricing.json`，不会保存 API 密钥。税费/关税只是操作者自行填写的估算，不是
税务建议；CatalogFlow 不会向海关或供应商自动查询税率。没有该文件时继续使用旧的默认利润率方案，
已有预览保持兼容。
